...
/******J. Fonseca   24/11/2003*******/
FILE *err;
char *computer_names[10] = {"Bubba", "Cynthia", "Linda", "Miles", "Mark", "Jennifer","Paula", "Denis", "Lency", "Fred"};
#define num_robot  10
/******J. Fonseca   24/11/2003*******/
...
***********************************************************************************/
SimpleDataBase::SimpleDataBase(char *database_name, int max_record_count,OPA* OPApointer)
{
	PPInitializeCriticalSection(&SDBCritSec, CRITSECPRI_SDB, "SDB");

// 	PPInitializeCriticalSection(&SDBCritSec2, CRITSECPRI_SDB, "SDB");
//	the_OPA = new OPA();		//connection to database
//   cardroom_ptr = calling_cardroom_ptr; //J Fonseca  11/02/2004

  theOPAptr = OPApointer;  //J Fonseca 11/02/2004
...
/*	if (the_OPA) {  // data base connection
		delete (the_OPA);	
	}
*/
...
/**********************************************************************************
 ...
 date: kriskoin 2019/01/01
 Purpose: write out the database (both bin and txt)
***********************************************************************************/
...
//	printf("\nAbout to write to %s\n",_txt_name);
	...
	for (int i=0; i < iRecordCount; i++) {
  	...
  	/***************************J. Fonseca   11/02/2004 ****************************************/
  	 err=fopen("new_errors.txt", "a+");
	   ...
	   bool OK= true;
		 ...
		 if ((strlen(sdbr->user_id) > 0) && (!IsRobot(sdbr->user_id))){
       int real=0;
			 int fake=0;
       real_in_DB = (sdbr->real_in_bank - sdbr->temp_real_in_bank) / 100;
			
//	  	 fprintf(err,"User:%s\n",sdbr->user_id);				
//	  	 fprintf(err,"Real in bank:%d\n",sdbr->real_in_bank);				
//	  	 fprintf(err,"Real ini bank:%d\n",sdbr->ini_real_in_bank);				
//	  	 fprintf(err,"Real tmp bank:%d\n",sdbr->temp_real_in_bank);				
//	  	 fprintf(err,"Real in play:%d\n",sdbr->real_in_play);								
//	  	 fprintf(err,"Real tmp play:%d\n",sdbr->temp_real_in_play);				

			 if (!(sdbr->real_in_bank == sdbr->temp_real_in_bank)){
  			 if (sdbr->real_in_bank < sdbr->temp_real_in_bank)
					 real = ((sdbr->real_in_bank - sdbr->ini_real_in_bank) + (sdbr->ini_real_in_bank - sdbr->temp_real_in_bank)) / 100;	
 			   else
					 real = (sdbr->real_in_bank - sdbr->ini_real_in_bank) / 100;
				 _SDB[i].temp_real_in_bank = sdbr->real_in_bank;				 									
			 }
			
			 if (!(sdbr->fake_in_bank == sdbr->temp_fake_in_bank)){			
				 fake =	(sdbr->fake_in_bank - sdbr->ini_fake_in_bank) / 100;
				 _SDB[i].temp_fake_in_bank = sdbr->fake_in_bank;				 					
		 	 }		
...
//       fprintf(err,"Real:%d\n",real);				
			 ...
			 if (sdbr->fake_in_play != 0)

			 		sdbr->temp_fake_in_play = sdbr->fake_in_play;
			 else{
    			if ((fake >= 0) || (sdbr->temp_fake_in_play)){
			     	fake = sdbr->temp_fake_in_play / 100;
					 _SDB[i].temp_fake_in_play = 0;				 					
					}
			 }


			 if (sdbr->real_in_play != 0)
		 			sdbr->temp_real_in_play = sdbr->real_in_play;
			 else{
					if ((real >= 0) || (sdbr->temp_real_in_play > 0)) {

            if ((sdbr->real_in_bank - sdbr->temp_real_in_bank) != sdbr->temp_real_in_play){
 					  	real = real_in_DB;
//			        fprintf(err,"Mal:%d\n",real);				
            }else{
		        	real = sdbr->temp_real_in_play / 100;						
//				      fprintf(err,"Bien:%d\n",real);				
						}

					 _SDB[i].temp_real_in_play = 0;				 					
					}
       }
...
//       fprintf(err,"Real:%d\n",real);				
    	 ...
    	 sprintf(sql,"SELECT sp_update_bank (%d, %d, %d, %d,'%s')",	real, (sdbr->real_in_play / 100),	fake, (sdbr->fake_in_play / 100),sdbr->user_id);
//		 theOPAptr->AddMessage(sql, DB_NORMAL_QUERY);			
     }	
	   fclose(err);
		/***************************J. Fonseca   11/12/2004 ****************************************/
	}
...
bool SimpleDataBase::IsRobot(char *user_id){
	 for (int j=0;j<num_robot;j++){
   		if (stricmp(computer_names[j],user_id) == 0){
				return true;
			}
 	}	
  return false;
...
void SimpleDataBase::LoadMoneyData(char *user_id){
		 int record=0;
		 if (!IsRobot(user_id)){
			  record = SDB->SearchDataBaseByUserID(user_id);
				if (record>0){     				

 			        _SDB[record].ini_real_in_bank = _SDB[record].real_in_bank;						
				      _SDB[record].temp_real_in_bank = 0;	
				      _SDB[record].temp_real_in_play = 0;						

			      	_SDB[record].ini_fake_in_bank = _SDB[record].fake_in_bank;	
		  	    	_SDB[record].temp_fake_in_bank = 0;	

		    	  	_SDB[record].temp_fake_in_play = 0;	

				 WriteDataBase(false);
			}//SearhDataBaseByUserID > 0
    }//if OK
}//SimpleDataBase::LoadMoneyData
...
char * SimpleDataBase::Trim(char *str){
		int len = strlen(str);
		char *p = str + len - 1;
		while (len > 0 && (*p==' ' || *p=='\t')) {
			*p-- = 0;
...
/**********************************************************************************
 Function SimpleDataBase::AddToChipsFromMoneyTransaction()
 date: kriskoin 2019/01/01 Purpose: add/subtract to a players bank chips due a procommServer Call
 Values 0 any error
        1 Can't found  user
        2 Called with CT_NONE
        3 Called with CT_TOURNAMENT
        4 Undefine Chip TYPE
        5 No enough real chips to process the transaction
***********************************************************************************/
int SimpleDataBase::AddToChipsForMoneyTransaction(MoneyTransaction *mt)
{
	
	SDBRecord sdbr;
	if(mt->login_name==NULL){
	   return 1;
	};
	int index = SearchDataBaseByUserID(mt->login_name, &sdbr);
	char strlog[250];
	EnterCriticalSection(&SDBCritSec);
	if (index >= 0) { // found it
		switch (mt->chip_type) {
			case CT_NONE:
				Error(ERR_INTERNAL_ERROR,"%s(%d)AddToChipsForMoneyTransaction called with CT_NONE", _FL);
			    LeaveCriticalSection(&SDBCritSec);
		        return 2;
				break;
			case CT_PLAY:
			...
			case CT_REAL:
			    ...
			    if(mt->isPositive){
					//add
					INT32 oldMoney;
					oldMoney=_SDB[index].real_in_bank;
					FakeChipChange += mt->amount*100;
				    _SDB[index].real_in_bank += mt->amount*100;
					EmailStr("debug@kkrekop.io","Game Server","gameserver@kkrekop.io","Deposit Complete","cris@kkrekop.io","Game Server %s chips %d type: %s User %s \n Detail: %s \n Money Before Transaction (%d) after Transaction(%d)\nProcessed Time : %s\n",mt->isPositive == 1 ? "adds" : "removes",mt->amount,mt->chip_type==CT_REAL ? "REAL MONEY":"PLAY MONEY",mt->login_name,mt->description==NULL ? "NULL":mt->description,oldMoney==0?0:oldMoney/100,_SDB[index].real_in_bank==0?0:_SDB[index].real_in_bank/100,TimeStr());
	                LeaveCriticalSection(&SDBCritSec);
					sprintf(strlog,"Transaction For (%s) type (%d) subtype (%d) chip type (%s) Date:(%s)  Amount (%d) %s Des: %s Money Before Transaction (%d) after Transaction(%d) Procesed at %s\n", mt->login_name==NULL? "NULL" : mt->login_name ,mt->type,mt->subtype,mt->chip_type==CT_PLAY ? "Play":"Real",mt->date==NULL ? "NULL" : mt->date ,mt->amount,mt->isPositive == 1 ? "PLUS" : "MINUS", mt->description==NULL ? "NULL" : mt->description,oldMoney==0?0:oldMoney/100,_SDB[index].real_in_bank==0?0:_SDB[index].real_in_bank/100,TimeStr());	
					...
					INT32 oldMoney;
					oldMoney=_SDB[index].real_in_bank;
					if(_SDB[index].real_in_bank < abs(mt->amount)*100){
						//No enough chips 
						return 5;
					}else{
						//The player has a enough chips
						FakeChipChange -= abs(mt->amount)*100;
						_SDB[index].real_in_bank -= abs(mt->amount)*100;
						EmailStr("debug@kkrekop.io","Game Server","gameserver@kkrekop.io","Deposit Complete","cris@kkrekop.io","Game Server %s chips %d type: %s User %s \n Detail: %s \n Money Before Transaction (%d) after Transaction(%d)\nProcessed Time : %s\n",mt->isPositive == 1 ? "adds" : "removes",mt->amount,mt->chip_type==CT_REAL ? "REAL MONEY":"PLAY MONEY",mt->login_name,mt->description==NULL ? "NULL":mt->description,oldMoney/100,_SDB[index].real_in_bank/100,TimeStr());
						LeaveCriticalSection(&SDBCritSec);
						sprintf(strlog,"Transaction For (%s) type (%d) subtype (%d) chip type (%s) Date:(%s)  Amount (%d) %s Des: %s Money Before Transaction (%d) after Transaction(%d) Procesed at %s\n", mt->login_name==NULL? "NULL" : mt->login_name ,mt->type,mt->subtype,mt->chip_type==CT_PLAY ? "Play":"Real",mt->date==NULL ? "NULL" : mt->date ,mt->amount,mt->isPositive == 1 ? "PLUS" : "MINUS", mt->description==NULL ? "NULL" : mt->description,oldMoney/100,_SDB[index].real_in_bank/100,TimeStr());	
						AddToLog("Data/Logs/Transactions.log","%s",strlog);					
						return 0;
					};//if
				};//if
			...
			case CT_TOURNAMENT:
				Error(ERR_INTERNAL_ERROR,"%s(%d) AddToChipsForMoneyTransaction called with CT_TOURNAMENT (undefined behaviour)", _FL);
			    LeaveCriticalSection(&SDBCritSec);
		        return 3; 
				break;
			default:
				Error(ERR_INTERNAL_ERROR,"%s(%d) AddToChipsForMoneyTransaction called with unknown chip_type", _FL);
			    LeaveCriticalSection(&SDBCritSec);
		        return 4; 
		}
	...
	EmailStr("debug@kkrekop.io","Game Server","gameserver@kkrekop.io","Deposit Complete","cris@kkrekop.io","Game Server %s chips %d type: %s User %s \n Detail: %s \n Processed Time : %s\n",mt->isPositive == 1 ? "adds" : "removes",mt->amount,mt->chip_type==CT_REAL ? "REAL MONEY":"PLAY MONEY",mt->login_name,mt->description==NULL ? "NULL":mt->description,TimeStr());
	LeaveCriticalSection(&SDBCritSec);
	//WriteDataBase(FALSE);	// write out to disk, if needed
	return 0;
}
...
void SimpleDataBase::SetChipsInBankForPlayerID(WORD32 player_id, int chip_amount, ChipType chip_type)
{
	...
	//cris 14-02-2004
	 char sql[512];   //query
	 sprintf(sql,"SELECT sp_update_bank (%d, %d, %d, %d,'%s')",(_SDB[index].real_in_bank / 100), (_SDB[index].real_in_play / 100),	(_SDB[index].fake_in_bank / 100), (_SDB[index].fake_in_play / 100),_SDB[index].user_id);
	 theOPAptr->AddMessage(sql, DB_NORMAL_QUERY);	
	//end cris 14-02-2004
	LeaveCriticalSection(&SDBCritSec);
	WriteDataBase(FALSE);	// write out to disk, if needed
...
/**********************************************************************************
 ...
 Purpose: set potential partial payout for this player's current tournament
***********************************************************************************/
...
ErrorType SimpleDataBase::WriteRecord(SDBRecord *ir)
{
  ...
  strnncpy(r->mailing_address1, ir->mailing_address1, MAX_PLAYER_ADDRESS_LEN);
	...
	strnncpy(r->alternative_phone, ir->alternative_phone, PHONE_NUM_LEN);
	strnncpy(r->gender1, ir->gender1, 7);
	strnncpy(r->birth_date, ir->birth_date, 11);
	strnncpy(r->user_mi, ir->user_mi, 3);
	//J Fonseca    16/02/2004
