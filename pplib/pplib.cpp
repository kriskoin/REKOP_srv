...
//cris Tue Feb 17 14:04:41 2004
char *TimeStr2(void)

{

	return TimeStr2(time(NULL), FALSE, FALSE, 0);

}

char *TimeStrWithYear2(void)

{

	return TimeStr2(time(NULL), TRUE, FALSE, 0);
...

{
	...
	if (year_flag) {
		...
		sprintf(time_str[buf_index], "%04d-%02d-%02d_%02d:%02d:%02d", t->tm_year+1900, t->tm_mon+1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);

	} else {

		sprintf(time_str[buf_index], "%02d-%02d_%02d:%02d:%02d", t->tm_mon+1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
...
//end cris Tue Feb 17 14:04:54 2004
...

{
	...
	forever {
		...
		if (eq) {
			...
			// test: metasend -f /tmp/home.html -s "html test" -t "test@TropicanaPoker.com" -F "test@TropicanaPoker.com" -c "" -e 7bit -m text/richtext -b
